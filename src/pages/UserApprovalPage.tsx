...
export default function UserApprovalPage() {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
      ...
      {/* Header Banner */}
      <div className="bg-gradient-to-r from-slate-900 via-sky-950 to-slate-900 text-white rounded-3xl p-6 sm:p-8 shadow-xl relative overflow-hidden border border-slate-800">
        <div className="absolute right-0 top-0 bottom-0 opacity-10 pointer-events-none flex items-center pr-8">
          <ShieldAlert className="w-64 h-64 text-sky-400" />
        </div>

        <div className="relative z-10 space-y-3">
          <div className="inline-flex items-center gap-2 bg-sky-500/20 border border-sky-400/30 px-3 py-1 rounded-full text-sky-300 text-xs font-bold uppercase tracking-wider">
            <Shield className="w-3.5 h-3.5" />
            <span>Super Admin Governance Control</span>
          </div>

          <h1 className="text-2xl sm:text-3xl font-black tracking-tight text-white">
            Aktivasi User Aktif & Approval Portal
          </h1>
          <p className="text-slate-300 text-xs sm:text-sm max-w-2xl font-medium leading-relaxed">
            Pusat persetujuan pendaftaran akun internal Pancaran Logistic. Kelola hak akses login, tinjau registrasi pengguna baru, dan kontrol otorisasi akun super admin & staf.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div 
          ...
          className={`bg-amber-50/50 dark:bg-amber-950/30 border rounded-2xl p-5 shadow-sm hover:shadow-md transition-all cursor-pointer relative overflow-hidden ${
            filterStatus === "pending" ? "border-amber-500 ring-2 ring-amber-500/30" : "border-amber-200 dark:border-amber-900/50"
          }`}
        >
          {pendingCount > 0 && (
            <span className="absolute top-3 right-3 w-3 h-3 bg-amber-500 rounded-full animate-ping" />
          )}
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-extrabold text-amber-800 dark:text-amber-400 uppercase tracking-wider">Pending Approval</span>
            <Clock className="w-5 h-5 text-amber-600 dark:text-amber-400" />
          </div>
          <div className="text-2xl sm:text-3xl font-black text-amber-600 dark:text-amber-400">{pendingCount}</div>
          <p className="text-[11px] font-bold text-amber-700 dark:text-amber-300 mt-1">Menunggu persetujuan Super Admin</p>
        </div>
                  ...
                  return (
                    <tr 
                      ...
                      className={`hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors ${
                        user.status === "pending" ? "bg-amber-50/20 dark:bg-amber-950/10" : ""
                      }`}
                    >
                      {/* Name & Email */}
                      ...
                      <td className="py-4 px-4">
                        ...
                        {user.status === "pending" && (
                          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-extrabold bg-amber-100 text-amber-900 dark:bg-amber-950/80 dark:text-amber-300 border border-amber-300 dark:border-amber-800 animate-pulse">
